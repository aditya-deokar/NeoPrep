...
export const GenerateAndSaveAllChapters = async (course: any) => {
  try {
    ...
    const chapterOperations = await Promise.all(
      chapters.map(async (chapter: any, index: number) => {
        let videoId = "";
        let content;

        try {
          content = await GenerateChapterContentLayout(courseName, chapter);
          if (typeof content === "string") {
            content = JSON.parse(content);
          }

          const videoResp = await getVideo(`${courseName}:${chapter.ChapterName}`);
          videoId = videoResp[0]?.id?.videoId || "";
        } catch (err) {
          console.error(`Error preparing chapter ${index + 1}:`, err);
          throw new Error(`Failed to prepare chapter ${index + 1}`);
        }
    ...
    // Execute all creates as a single atomic transaction
    await prisma.$transaction([
      ...chapterOperations,
      prisma.courseList.update({
        where: { id: course.id },
        data: { publish: true },
      }),
    ]);
    ...
    throw new Error(error.message || "Failed to generate and save all chapters.");
  }
};
